...
lrsoltype(::DenseVector{T}) where {T} = Vector{T}
lrsoltype(::DenseMatrix{T}) where {T} = Matrix{T}

_vaug(X::DenseMatrix{T}) where {T} = vcat(X, ones(T, 1, size(X,2)))::Matrix{T}
_haug(X::DenseMatrix{T}) where {T} = hcat(X, ones(T, size(X,1), 1))::Matrix{T}


## linear least square

function llsq(X::DenseMatrix{T}, Y::DenseVecOrMat{T};
...
function ridge(X::DenseMatrix{T}, Y::DenseVecOrMat{T}, r::Real;
              trans::Bool=false, bias::Bool=true) where T<:AbstractFloat
...
function ridge(X::DenseMatrix{T}, Y::DenseVecOrMat{T}, r::DenseVector{T};
              trans::Bool=false, bias::Bool=true) where T<:AbstractFloat
...
function ridge(X::DenseMatrix{T}, Y::DenseVecOrMat{T}, r::DenseMatrix{T};
              trans::Bool=false, bias::Bool=true) where T<:AbstractFloat
    d = lreg_chkdims(X, Y, trans)
...
function _ridge(X::DenseMatrix{T}, Y::DenseVecOrMat{T},
                r::Union{Real, DenseVecOrMat}, trans::Bool, bias::Bool) where T<:AbstractFloat
    if bias
        if trans
            X_ = _vaug(X)
